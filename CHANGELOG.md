...
### Fixed
- `SceneLoader`: duplicate instance detection now calls `Destroy(this)` instead of `Destroy(gameObject)`. The previous behavior destroyed the entire GameObject, which would wipe out the HUD or any other components sharing the same root when a bootstrap scene already provided a persistent `SceneLoader`.
